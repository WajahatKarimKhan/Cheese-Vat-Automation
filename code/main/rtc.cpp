...
void initRTC() {
  ...
  URTCLIB_WIRE.begin();
  //comment the following after setting the time
  // once time is set,again upload the code so when reset pressed the system keep the date/time saved
...
bool checkRTC(){
  rtc.refresh();
  if (rtc.hour()> 24 || rtc.minute()> 59 || rtc.year()== 0 ){
      rtc_working = false;
    } else {
      rtc_working = true; 
    }
