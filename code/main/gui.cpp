...
void drawProgrammingScreen(){
  ...
  */
  // --- Programming - Milk Refill Button ---
  myGLCD.setColor(0, 102, 204);  // Blue
  myGLCD.fillRoundRect(540, 175, 770, 275);  // x1, y1, x2, y2
  myGLCD.setColor(255, 255, 255);
  myGLCD.setBackColor(0, 102, 204); // Text background also blue
  myGLCD.drawRoundRect(540, 175, 770, 275);
  drawBoxCenteredText("Milk Refill", 540, 175, 770, 275);
...
void drawAP_MilkRefillScreen(){
  ...
  myGLCD.setFont(BigFont);
  myGLCD.setColor(0, 102, 204);  // Blue
  myGLCD.fillRoundRect(125, 160, 325, 240);  // x1, y1, x2, y2
  myGLCD.setColor(255, 255, 255);
  myGLCD.setBackColor(0, 102, 204); // Text background also blue
  myGLCD.drawRoundRect(125, 160, 325, 280);
  drawBoxCenteredText("Target", 125, 163, 325, 185);
  drawBoxCenteredText("Milk", 125, 188, 325, 210);
  drawBoxCenteredText("Refill (ltr)", 125, 213, 325, 235);
  myGLCD.print(milkRefillProgram.milkinltrs, 186, 252);
  // --- Live Data Button ---
  myGLCD.setFont(BigFont); 
  myGLCD.setColor(255, 102, 102);  // red
  myGLCD.fillRoundRect(620, 230, 770, 330);  // x1, y1, x2, y2
...
void drawPRG_PasteurizationScreen(){
  ...
  // --- save Data Button --- 
  drawSaveButton(180, 410, 280, 460);
...
void drawPRG_CoolingScreen(){
  ...
  // --- save Data Button --- 
  drawSaveButton(180, 360, 280, 410);
...
void drawPRG_MilkRefillScreen(){
  ...
  // Header
  myGLCD.setColor(255, 255, 255);
  myGLCD.print("Program MILK REFILL", CENTER, 30);
  myGLCD.drawLine(10, 50, 790, 50);
...
void drawRun_PasteurizationScreen(){
  ...
  //drawToggleButton(350, 195, stirrerDirection, "CW", "CCW");
...
void drawSaveButton(int x1, int y1, int x2, int y2) {
  myGLCD.setFont(BigFont);
  myGLCD.setColor(255, 102, 102);  // red
  myGLCD.fillRoundRect(x1, y1, x2, y2);
  myGLCD.setColor(255, 255, 255);
  ...
  myGLCD.drawRoundRect(x1, y1, x2, y2);
  drawBoxCenteredText("Save", x1, y1, x2, y2);
}
...
void draw_Axis_Temp1_Graph() {
  myGLCD.fillScr( 60, 60, 60);
  myGLCD.setBackColor(60, 60, 60);
  // Title
  myGLCD.setFont(BigFont);
  myGLCD.setColor(255, 255, 255);  // White text
  myGLCD.print(" Milk Live Temperature ", CENTER, 30);
  myGLCD.drawLine(10, 50, 790, 50);
  // Axes
  myGLCD.setColor(VGA_WHITE);
  myGLCD.drawRect(30, 50, 760, 450);  // Graph border

  myGLCD.setFont(SmallFont);
  myGLCD.print("Time (s)", 384, 460);
  // X-axis labels
  for (int t = 0; t <= MAX_POINTS; t += 10) {
  int x = mapTimeToX(t);
  myGLCD.printNumI(t, x - 5, 455);  // Time labels
  }
  // Y-axis labels
  myGLCD.print("Temp (C)", 10, 50);
  for (int t = 0; t <= 110; t += 10) {
  int y = mapTempToY(t);
  myGLCD.printNumI(t, 0, y - 4);  // Temperature labels
  }
  drawBackButton();
}

void drawHoldElapsedTime(unsigned long holdStartTime, int x, int y) {
  static unsigned int lastSeconds = 61;  // Force first update

  unsigned long elapsed = millis() - holdStartTime;
  unsigned int mins = elapsed / 60000UL;
  unsigned int secs = (elapsed % 60000UL) / 1000UL;

  // Avoid redrawing unless value changes
  if (secs != lastSeconds) {
    lastSeconds = secs;
    ...
    myGLCD.setColor(VGA_WHITE);
    myGLCD.setBackColor(60, 60, 60);  // Match background
    myGLCD.setFont(SmallFont);
    myGLCD.print("        ", x, y);  // Clear previous
    myGLCD.print(buffer, x, y);            // Draw updated
  }
}
