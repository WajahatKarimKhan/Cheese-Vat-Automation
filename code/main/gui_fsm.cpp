...
//Programming mode Screens
    ...
    case GUI_PRG_COOLING_SCREEN:
      ...
      break;
      case GUI_PRG_MILK_REFILL_SCREEN:
      handlePRG_MilkRefillScreen();
      break;
//Manual Mode Screens
...
//Live data + graphs
    case GUI_LIVE_DATA_SCREEN:
      ...
      break;
    case GUI_TEMP_1_GRAPH_SCREEN:
      handle_Temp1_Graph_Screen();
      break;
      ///adding states
  }
}
...
void handleProgrammingScreen() {
  ...
  /*/if COOLING button is pressed
    ...
    //if milk refill button is pressed - will use it when needed auto milk refilling
    if ((x >= 540) && (x <= 770) && (y >= 175) && (y <= 275)) {
      drawFrame(540, 175, 770, 275);
      Run_cmd = GUI_PRG_MILK_REFILL_SCREEN;
      myGLCD.clrScr();
      initialized = false;
    }      
    // if Back button pressed
...
void handlePRG_MilkRefillScreen(){
  activeProgram = &milkRefillProgram;
  static bool initialized = false;
  if (!initialized) {
    drawPRG_MilkRefillScreen();
    initialized = true;
  }
  if (myTouch.dataAvailable()) {
    myTouch.read();
...
void handleMan_CoolingScreen(){
  ...
  // Rest of the code is in the manual_data.cpp that handles data inputs from user 
    ...
    if ((x >= 470) && (x <= 560) && (y >= 360) && (y <= 410)) {
      ...
      Run_cmd = GUI_RUN_COOLING_SCREEN; 
      myGLCD.clrScr();
      //executed through the pasteurization or cheese making
      initialized = false;
    }
    //if stop button is pressed
    if ((x >= 670) && (x <= 770) && (y >= 360) && (y <= 410)) {
...
void handleRun_PasteurizationScreen(){
  ...
  if (pastPhase == PAST_PHASE_HOLDING) { drawHoldElapsedTime(past_holdStart, 355, 120); }
  drawToggleButton(350, 195, stirrerDirection, "CW", "CCW");
  drawToggleButton(350, 155, stirrerRunning, "ON", "OFF");
  //drawLiveDate(  0, 460, 205, 480);
  drawLiveTime(670, 0, 800, 40);
  static bool initialized = false;
  if (!initialized) {
...
void handleRun_CheeseMakingScreen(){
  ...
  //drawFloatLive(TEMP_2.readCelsius(), 300, 115, 370, 140);
  //drawFloatLive(TEMP_3.readCelsius(), 300, 155, 370, 180);
  if (cmPhase == CM_PHASE_HOLDING) { drawHoldElapsedTime(cm_holdStart, 355, 120); }
  drawToggleButton(350, 195, stirrerDirection, "CW", "CCW");
  drawToggleButton(350, 155, stirrerRunning, "ON", "OFF");
  drawLiveTime(670, 0, 800, 40);
  static bool initialized = false;
...
void handleLiveDataScreen(){
  ...
  if (myTouch.dataAvailable()) {
    ...
    x = myTouch.getX();
    y = myTouch.getY();
    // if graph for temp1  button pressed
    if ((x >= 420) && (x <= 520) && (y >= 75) && (y <= 100)) {
      drawFrame(420, 75, 520, 100);
      Run_cmd = GUI_TEMP_1_GRAPH_SCREEN;     // Go back to the screen we came from
      myGLCD.clrScr();
      initialized = false; 
    } 
    // if Back button pressed
...
void handle_Temp1_Graph_Screen(){
  update_Temp1_Graph();
  live_Temp1_Graph();
  drawLiveTime(670, 0, 800, 40);
  //update_Temp1_Graph();
  drawFloatLive(TEMP_1.readCelsius(), 360, 75, 440, 100);
  static bool initialized = false;
  if (!initialized) {
    draw_Axis_Temp1_Graph();
    initialized = true;
  }
  if (myTouch.dataAvailable()) {
    myTouch.read();
    ...
    if ((x >= 0) && (x <= 140) && (y >= 0) && (y <= 50)) {
      drawFrame(10, 10, 60, 36);
      Run_cmd = GUI_LIVE_DATA_SCREEN;     // Go back to the screen we came from
      myGLCD.clrScr();
      initialized = false;
    }
